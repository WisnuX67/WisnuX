...
if not WisnuLib then warn("[WisnuHub] UI gagal load"); return end
local Library = WisnuLib -- compatibility alias
...
local Tabs = {
    ...
    UISettings = Window:CreateTab({Name = "UI Settings"})
}
...
local function TeleportToPart(part)
    ...
    if hrp then
        ...
        WisnuLib:SetNotification({Content = "Teleport: Berhasil!", Delay = 1})
    end
end
...
local function createStatusESP(player, char, root)
    ...
    local text = ""
    if isDown then text = "Ã°Å¸â€Â» DOWN\n" end
...
local function TeleportToGenerator()
    ...
    if #gens == 0 then 
        WisnuLib:SetNotification({Content = "TP Generator: Tidak ada generator!", Delay = 2})
        return 
    end
    ...
    if part then
        TeleportToPart(part)
        WisnuLib:SetNotification({Content = "TP Generator: Generator ", Delay = 1.2})
    end
...
local function TeleportToHook()
    ...
    if #hooks == 0 then 
        WisnuLib:SetNotification({Content = "TP Hook: Tidak ada hook!", Delay = 2})
        return 
    end
...
local function TeleportToGate()
    ...
    if #gates == 0 then 
        WisnuLib:SetNotification({Content = "TP Gate: Tidak ada gate!", Delay = 2})
        return 
    end
...
local function TeleportToPallet()
    ...
    if #pallets == 0 then 
        WisnuLib:SetNotification({Content = "TP Pallet: Tidak ada pallet!", Delay = 2})
        return 
    end
...
local function TeleportToWindow()
    ...
    if #windows == 0 then 
        WisnuLib:SetNotification({Content = "TP Window: Tidak ada window!", Delay = 2})
        return 
    end
...
local function RefreshMapForTeleport()
    ...
    WisnuLib:SetNotification({Content = "Refresh Map: Cache diperbarui!", Delay = 2})
end
...
AbilityTab:AddCheckbox("GenBypassToggle", {
    ...
    Callback = function(v) 
        setGenBypass(v)
        if v then
            WisnuLib:SetNotification({Content = "Gen Bypass: Diaktifkan - Klik tombol BYPASS di layar (mobile)", Delay = 3})
        end
    end
})
...
KillerTab:AddCheckbox("BypassCooldown", {
    ...
    Callback = function(state)
        ...
        if state then
            StartCooldownBypass()
            WisnuLib:SetNotification({Content = "Bypass Cooldown: Diaktifkan", Delay = 3})
        else
            StopCooldownBypass()
            WisnuLib:SetNotification({Content = "Bypass Cooldown: Dinonaktifkan", Delay = 3})
        end
    end
})
...
local FakeParryToggle = ParryBox:AddToggle({
...
MorphAvaBox:AddButton({
    ...
    Callback = function()
        ApplyKorless()
        WisnuLib:SetNotification({Content = "Notification", Delay = 2})
    end
})
...
SettingBox:AddButton({Title = "Unload script", Callback = function() end})

-- ThemeManager (removed)
-- SaveManager (removed)
-- SaveManager (removed)
-- SaveManager (removed)
-- ThemeManager (removed)
